    ...
    function deleteConfirm(){
       ...
       <?php
        ...
        if(isset($_GET["function"])=='del'){
            if(isset($_GET["id"])){
                $orderid=$_GET["id"];
                pg_query($conn,"delete from orderdetail where OrderID='$orderid'")or die(pg_error($conn));
                pg_query($conn,"delete from `order` where OrderID='$orderid'")or die(pg_error($conn));
            }
        }
        ?>
...
<div class="container-fluid">
  ...
  <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block bg-light sidebar collapse">
    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      ...
      <h2>Orders</h2>
      <div class="add"><a href="add_order.php"><i class="fas fa-plus"></i>&nbsp;Add Orders</a></div>
      <div class="table-responsive">
        <table class="table table-striped table-sm">
          ...
          <?php
            $No=1;
            $result=pg_query($conn,"SELECT * FROM `order`") or die(pg_error($conn));
            while($row=pg_fetch_array($result,NULL,PGSQL_ASSOC))
            {
            ?>
            <tr>
              <td><?php echo $row["orderid"];?></td>
              <td><?php echo $row["customeridD"];?></td>
              <td><?php echo $row["orderdate"];?></td>
              <td><a href="update_order.php?function=del&&id=<?php echo $row["OrderID"];?>"><i class="fas fa-pencil-alt"></i></a></td>
